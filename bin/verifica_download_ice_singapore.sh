...
if [ $# -ne 1 ]; then
	echo "Uso: verifica_download_ice_singapore.sh <dias>"
...
cd $HOME/data/ice/singapore

problemas=""
for i in ice_singapore; do
...
if [ -n "$problemas" ]; then
	...
	echo $problemas
	mailx -s "Arquivos de ICE-Singapore para $data não baixados"  capta <<FIM
