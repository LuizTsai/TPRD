...
if [ $# -ne 1 ]; then
	echo "Uso: verifica_download_lme.sh <dias>"
...
problemas=""
for i in LME-non-ferrous LME-minor-metals LME-steel-rebar LME-steel-scrap; do
...
if [ -n "$problemas" ]; then
	...
	echo $problemas
	mailx -s "Arquivos de LME para $data não baixados"  capta <<FIM
