...
if [ $# -ne 1 ]; then
	echo "Uso: verifica_download_liffe_eur.sh <dias>"
...
cd $HOME/data/liffe/eur

problemas=""
for i in LIFFE-paris; do
...
if [ -n "$problemas" ]; then
	...
	echo $problemas
	mailx -s "Arquivos de LIFFE-Eur para $data não baixados"  capta <<FIM
