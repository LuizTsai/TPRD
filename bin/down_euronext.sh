...
euronext () {
	...
	baixa "https://www.euronext.com/sites/www.euronext.com/files/statistics/derivatives/daily/${ano}/${1}-DailyDerivativesStatistics%20${ano}-${mes}-${dia}.xlsx" "Euronext_${1}" "xls"
	...
	if [ $ret -eq 0 ]; then
	        ...
	        if [ -z "$fl" ]; then
        		fl=`file "tudo/Euronext_${1}_${data}_${download}.xls" | grep "Microsoft Excel 2007"`
			if [ -z "$fl" ]; then   
...
baixa "https://www.euronext.com/en/reports-statistics/derivatives/daily-statistics" "lixo" "html" "--save-cookies /tmp/cookies-euronext_1 --keep-session-cookies --referer=\"http://derivatives.euronext.com/trading/reports-statistics/daily-statistics\""
