...
if [ $# -ne 1 ]; then
	echo "Uso: verifica_download_lbma.sh <dias>"
...
problemas=""
for i in LBMA_Gold LBMA_Silver LBMA_Platinum LBMA_Palladium; do
...
if [ -n "$problemas" ]; then
	...
	echo $problemas
	mailx -s "Arquivos de LBMA para $data não baixados"  capta <<FIM
