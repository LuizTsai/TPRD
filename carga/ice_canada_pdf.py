...
#		else: return(0)
#	line = le_linha(f)
	while line and (re.match(r'\s*$', line) or re.match(r'^\s+DAILY PRICE RANGE\s+SETTLE\s+VOLUME AND OI TOTALS', line) or re.match(r'^\s+COMMODITY\s+CONTRACT', line) or re.match(r'^\s+NAME\s+MONTH', line) or re.match(r'^\s+TOTAL\s+BLOCK\s', line) or re.match(r'^\s+OPEN\#\s+HIGH\s+LOW\s+CLOSE\#\s+PRICE\s+CHANGE\s+OI\s+CHANGE\s+(ADJ\*\*)*\s+EFP\s+EFS', line) or re.match(r'^\s+VOLUME\s+VOLUME', line)):	
...
def tratar_vencimento(familia, tline):
	...
	if familia: # tratar vencimento
		...
		if not settle:
			print "!!!!AVISO: fechamento sem valor! (linha="+str(n_linha)+")"
		else:
			...
			if not ativo:
				...
				if not ativo:
					print "!!!!!!PROBLEMAS ao criar ativo para vencimento",venc,"linha=",n_linha
			if ativo: 
...
arquivo_log = admin.init("ice_canada")
programa_id = db.obtem_id_programa(debug, "ice_canada")
if not programa_id:
        print "PROBLEMAS!!: programa ice_canada não encontrado na base de dados!"
        # enviar email
        admin.sair(debug, programa_id, nome_arq, arquivo_log, data, count_i, count_u, count_p, "PROBLEMAS!!: programa ice_canada não encontrado na base de dados!");
...
line = le_linha(f)
if "Futures Daily Market Report for Canadian Grains" in line:
	...
	print "PROBLEMAS: Arquivo inválido: título \"Futures Daily Market Report for Canadian Grains\" não encontrado!"
       	admin.sair(debug, programa_id, nome_arq, arquivo_log, data, count_i, count_u, count_p, "Arquivo inválido: título \"Futures Daily Market Report for Canadian\" não encontrado")
