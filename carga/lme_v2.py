		...
		if ativo:
			...
			if not cotacao:
				...
				if (compra and compra>0 and cotacao.compra != compra) or (venda and venda>0 and cotacao.venda != venda) or (settle and settle>0 and cotacao.fechamento != settle):
					...
					if setlle and settle>0 and cotacao.fechamento != settle: print(".......mudou fechamento",(cotacao.fechamento,settle))
...
def trata_arquivo(debug, arquivo_log, nome_arq, programa_id, linha):
	...
	linha = re.sub(r'<h2>LME [Oo]fficial [Pp]rices', "<h2>LME Official prices", linha)
	...
	# obter Data
	f = f + len("</title>")
	i = linha.find("<div class=\"delayed-date left \">", f)
	if i == -1:
		print("PROBLEMAS: <div class=\"delayed-date left \"> não encontrado")
		print(linha[f:fim])
		admin.sair(debug, programa_id, nome_arq, arquivo_log, data, count_i, count_u, count_p, "PROBLEMAS: \"<div class=\"delayed-date left \">\"  não encontrado")
	i = i + len("<div class=\"delayed-date left \">")
	f = linha.find("</div>", i)
	if f == -1:
		print("PROBLEMAS: terminador (\"</div>\") da Data não encontrado")
		admin.sair(debug, programa_id, nome_arq, arquivo_log, data, count_i, count_u, count_p, "PROBLEMAS: terminador (\"</div>\") da Data não encontrado")
	str_data = linha[i:f]
	data=re.match(r'^(Data valid for|US\$/tonne for) ([0-9]+ [a-zA-Z]+ [0-9]+)$', str_data)
	if data is None:
	...
	if debug>2: print("Data encontrada:", data.groups())
	data = subprocess.check_output(["date", "-d "+data.group(2)," +%F"]).decode("utf-8")
	print("Arquivo contém dados de", data)
	
	
	### Obter dados
	f = f + len("</div>")
	i = linha.find("<h2>LME Official prices, US$ per tonne</h2><div class=\"table-wrapper\" data-arm-module=\"app/widget/ScrollableTable\">", f)
	if i == -1:
		print("PROBLEMAS: div com dados (id=\"<h2>LME Official prices, US$ per tonne</h2>\") não encontrado")
		print(linha[f:fim])
		admin.sair(debug, programa_id, nome_arq, arquivo_log, data, count_i, count_u, count_p, "PROBLEMAS: div com dados (id=\"<h2>LME Official prices, US$ per tonne</h2>\") não encontrado")
	f = f + len("<h2>LME Official prices, US$ per tonne</h2><div class=\"table-wrapper\" data-arm-module=\"app/widget/ScrollableTable\">")
