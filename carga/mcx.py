...
line = le_linha(f)
if re.match(r'Date,Commodity Symbol,Contract/Expiry Month,Open\(Rs\),High\(Rs\),Low\(Rs\),Close\(Rs\),PCP\(Rs\),Volume\(In Lots\),Volume\(In 000\'s\),Value\(In Lakhs\),OI\(In Lots\)', line):
	versao = 1
elif re.match(r'.*Date,Symbol,Expiry Date,Open,High,Low,Close,Previous Close,Volume,Volume\(In 000\'s\),Value,Open Interest', line):
	versao = 2
else:
