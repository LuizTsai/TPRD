...
arquivo_log = admin.init("cme_agro")
programa_id = db.obtem_id_programa(debug, "cme_agro")
if not programa_id:
...
line = le_linha(f)
if "FINAL PRE-CLEARING PRICES AS OF " in line:
	if debug: print("Arquivo OK")
	# obter data de referencia
	tdata = re.search(r'FINAL PRE-CLEARING PRICES AS OF (\d+)/(\d+)/(\d+) ', line)
	if tdata:
		dia = tdata.group(2)
		mes = tdata.group(1)
		ano = tdata.group(3)
...
if "STRIKE            OPEN         HIGH          LOW         LAST         SETT         CHGE     EST.VOL           SETT         VOL         INT" not in line:
